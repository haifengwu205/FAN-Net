...
# Import the os path module at the beginning:
...
def crf1(x, y, z):
    ...
    if use_2d:
        ...
        feats = create_pairwise_bilateral(sdims=(80, 80), schan=(13, 13, 13), #50,20
                                          img=img, chdim=2)
...
def crf2(x,y,z):
    ...
    if use_2d:
        ...
        # get unary potentials (neg log probability)
        U = unary_from_labels(labels, n_labels, gt_prob=0.7, zero_unsure=HAS_UNK) #0.7
        d.setUnaryEnergy(U)

        # This creates the color-independent features and then add them to the CRF
        feats = create_pairwise_gaussian(sdims=(3, 3), shape=img.shape[:2])#3,3;10,10
        d.addPairwiseEnergy(feats, compat=3,
                            kernel=dcrf.DIAG_KERNEL,
                            normalization=dcrf.NORMALIZE_SYMMETRIC)

        # This creates the color-dependent features and then add them to the CRF
        feats = create_pairwise_bilateral(sdims=(3, 3), schan=(5, 5, 5),#80,80 13;50,50 20
                                          img=img, chdim=2)
        d.addPairwiseEnergy(feats, compat=10,
                            kernel=dcrf.DIAG_KERNEL,
                            normalization=dcrf.NORMALIZE_SYMMETRIC)
...
def crf3(x,y,z):
    ...
    if use_2d:
        ...
        # get unary potentials (neg log probability)
        U = unary_from_labels(labels, n_labels, gt_prob=0.9, zero_unsure=HAS_UNK)  #0.7
        d.setUnaryEnergy(U)
        ...
        # This creates the color-dependent features and then add them to the CRF
        feats = create_pairwise_bilateral(sdims=(20, 20), schan=(20, 20, 20),#80,80 13;50,50 20
                                          img=img, chdim=2)
        d.addPairwiseEnergy(feats, compat=10,
                            kernel=dcrf.DIAG_KERNEL,
                            normalization=dcrf.NORMALIZE_SYMMETRIC)
...
def crf4(x,y,z):
    ...
    if use_2d:
        ...
        # This creates the color-dependent features and then add them to the CRF
        feats = create_pairwise_bilateral(sdims=(50, 50), schan=(20, 20, 20),#80,80 13;50,50 20
                                          img=img, chdim=2)
        d.addPairwiseEnergy(feats, compat=10,
                            kernel=dcrf.DIAG_KERNEL,
                            normalization=dcrf.NORMALIZE_SYMMETRIC)
...
def crf5(x, y, z, sdims1, compat1, sdims2, schan, compat2):
    fn_im = x
    fn_anno = y
    fn_output = z
    ...
    if use_2d:
        ...
        d.addPairwiseEnergy(feats, compat=compat1,  # 3
                            kernel=dcrf.DIAG_KERNEL,
                            normalization=dcrf.NORMALIZE_SYMMETRIC)

        # This creates the color-dependent features and then add them to the CRF

        feats = create_pairwise_bilateral(sdims=(sdims2, sdims2), schan=(schan, schan, schan),  # 50,20
                                          img=img, chdim=2)
